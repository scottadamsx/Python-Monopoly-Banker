...
def make_trade(currentPlayer, playerList):
    ...
    while True:
        ...
        property_choice = input("Select a property by number to offer (or 'done' to finish): ")
        if property_choice.lower() == 'done':
            break
        try:
    ...
    while True:
        ...
        property_choice = input("Select a property by number to ask (or 'done' to finish): ")
        if property_choice.lower() == 'done':
            break
        try:
            property_index = int(property_choice) - 1
    ...
    if trader_offer_value == tradee_ask_value:
        ...
        print(f"{tradee.name}'s ask is higher by ${tradee_ask_value - trader_offer_value}.")

    # Step 6: Tradee decides
    print(f"\n{tradee.name}, do you want to accept the offer, make a counteroffer, or decline?")
    decision = input("Enter 'accept', 'counter', or 'decline': ").lower()
